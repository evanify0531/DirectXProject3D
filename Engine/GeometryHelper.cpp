...
{
	float radius = 0.5f;
	uint32 stackCount = 20; // how many circles
	uint32 sliceCount = 20; // how many vertical slices
	...
	// for each circle, go around and calculate vertices (excluding poles)
...
void GeometryHelper::CreateSphere(shared_ptr<Geometry<VertexTextureNormalData>> geometry)
{
	...
	uint32 stackCount = 20; // horizontal
	uint32 sliceCount = 20; // vertical
	...
	// loop through circle between poles
	for (uint32 y = 1; y <= stackCount - 1; ++y)
	{
		float phi = y * stackAngle;
